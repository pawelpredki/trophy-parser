...
public class ExophaseGameParser {
    ...
    public void parseGame(Game game, int system) throws IOException {
        ...
        Document d = game.isLocal() ? Jsoup.parse(new File(url), "UTF-8") : Jsoup.connect(url).headers(JSOUP_HEADERS).get();
        ...
        if (hasPolish && doPolish) {
            d = game.isLocal() ? Jsoup.parse(new File(urlPl), "UTF-8") : Jsoup.connect(url + "pl").headers(JSOUP_HEADERS).get();
